...
extension BrewService {
    ...
    func searchPackages(query: String) async throws -> [Package] {
        ...
        let names = output.components(separatedBy: .whitespacesAndNewlines)
            ...
            .filter {
                !$0.isEmpty && !$0.contains("==>") && !$0.localizedCaseInsensitiveContains("Formulae")
    ...
    func fetchPackageSizes() async -> [String: Int64] {
        ...
        let await cellarPath =
            (try? run(arguments: ["--cellar"]))?
                .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        let await caskroomPath =
            (try? run(arguments: ["--caskroom"]))?
                .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
