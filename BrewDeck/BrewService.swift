...
class BrewService {
    ...
    init() {
        ...
        brewPath = paths.first(where: { FileManager.default.fileExists(atPath: $0) }) ?? "/opt/homebrew/bin/brew"
        setupAskPass()
    ...
    func run(arguments: [String], timeoutSeconds: Double = 60.0) async throws -> String {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            self.executeProcess(arguments: arguments, timeoutSeconds: timeoutSeconds, continuation: continuation)
        }
    }
}
